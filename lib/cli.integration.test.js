...
test('lint `defaultFiles`', async (t) => {
  const [,, stderr] = await execLinter({})
  const failedRuleIds = resultsToRuleIds(JSON.parse(stderr))
  t.deepEqual(failedRuleIds, ['no-unused-expressions'])
})
